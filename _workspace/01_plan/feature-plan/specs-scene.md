...
**동작 명세**: Lan* 피스 구간에서 레인 좌우 이동을 3D 뷰와 플라이스루 양쪽에서 시각적으로 구분해 표현한다. **기하 정본은 `_workspace/02_design/piece-geometry.md` §레인 기하다** — 레인 3개·각 12cm·전체 36cm, 레인은 선이 아니라 **면**(좌·우 가장자리를 중심선과 같은 규칙으로 생성), 한 칸씩 순환(0→1, 1→2, 2→0)하며 자리바꿈 곡선은 `(1−cos πt)/2`, 두 칸 건너뛰는 레인은 높이 8cm `sin²(πt)` **육교**다. 그 문서에 트랙 폭이 24cm로 그려지던 결함 이력(바깥 반 레인이 빠짐 — 화면으로는 안 보이고 숫자로만 드러난다)이 함께 있으니 `TRACK_WIDTH` 상수와 실제 렌더 폭을 대조하는 단언을 둔다. 고도 변화는 없다 — `laneOffset`은 고도와 독립된 수평축 변형이며 FEAT-005는 이것을 만들지 않는다(`elevation/types.ts`). 텍스트 구간 목록(TC-008-3)에서의 표기는 FEAT-013이 소유한 표면 위에 레인체인지를 한 세그먼트 유형으로 얹는 것이며, 목록 자체의 렌더·조작 책임은 FEAT-013에 있다.
