...
<!-- web-harness:unit feat=FEAT-001 dependsOn=none paths=api,src/features/load-track,src/widgets/app-header -->
...
<!-- web-harness:unit feat=FEAT-002 dependsOn=FEAT-001 paths=src/entities/track/lib/parse -->
...
<!-- web-harness:unit feat=FEAT-003 dependsOn=FEAT-002 paths=src/entities/track/lib/restore -->
...
② **매달린 끝이 정확히 둘이면 서로 잇는다** — 이을 곳이 하나뿐이라 모호함이 없다. 매달린 끝은 이웃(1px)이 없는 끝뿐 아니라 **끼어든 끝**(D-051: 자기는 정확 무리(0.02px)에 없고, 이웃들이 속한 정확 무리가 나를 빼고 짝수라 그들끼리 다 짝지어지는 끝)도 포함한다 — 실측 R84APY에서 코너 끝이 다른 이음새(정확 일치 쌍)에 0.285px 붙어 있어 실제 짝(16px 벌어진 직선)과 잇지 못했다. 홀수 무리(참조 트랙 입체교차 p47·p51·p52 + p38 0.17px)는 하나가 남으므로 그 끝이 짝이다.
...
④ **막히면 되돌아간다** — 방향만 보고 전진하면 입체교차에서 갇힌다. 실측 근거(PC-005): 순진한 끝점 매칭은 132개 중 **8개**만 복원했고, 현(chord) 방향 판별을 더해도 **123개**에서 멈췄다 — 45° 코너에서는 현 방향이 진입점의 접선과 다르기 때문이다. 끝점 264개 중 **7곳이 입체교차**(끝점 4개가 한 점), 2곳이 고립이다.
...
- TC-003-7: Given 이음새 하나가 16px 벌어지고 그 한쪽 끝이 다른 정확 일치 이음새에 0.285px 붙은 실측 트랙(R84APY), When 순서 복원을 수행하면, Then 끼어든 끝을 매달린 끝으로 세어 벌어진 이음새를 잇고 112피스 폐곡선을 낸다(D-051). 참조 트랙(WS67Y2)의 132피스 순서는 변하지 않는다(정확히 겹친 홀수 무리 옆의 끝은 매달린 끝이 아니다).
...
- TC-004-7: Given 순서 복원이 실패한 트랙, When 이어진 구간을 구하면, Then START에서 화살표 방향과 반대 방향 **양쪽**으로 걸은 사슬(뒤쪽을 뒤집어 앞에 붙임, D-050)이며 사슬은 START를 화살표 방향으로 통과한다. 요약에 "START 앞 n·뒤 m피스"가 남고, 전 피스가 이어졌는데 닫히지 않으면 배너가 "트랙이 닫히지 않아 이어진 사슬 전체를 표시했습니다"로 갈린다.
...
**동작 명세**: 복원된 경로의 각 피스에 타입별 고도 프로파일 함수를 적용해 피스 내부를 곡면 세그먼트로 변환한다. 고도는 **현(chord) 규칙**으로 정한다(D-022, 사용자 지정 렌더 규칙 — 실물 실측 아님): 피스의 시작점과 끝점을 이은 직선이 슬로프는 **20°**(D-025 → D-042), 뱅크는 **20°**(D-042에서 공식 실측값으로 복귀)를 이루도록 `H = 진행축 px × sin(각도)`로 총 상승량을 계산한다(D-023 — 진행축 px는 차가 달린 거리, 즉 빗변이다). 상승 뱅크와 하강 뱅크 **사이 구간**은 **하나의 20° 기운 평면(판)** 위에 놓이고, 트랙이 그 판 위를 돌면서 올라갔다 내려온다(D-029). **뱅크 피스 자체는 판이 아니라 전이 곡선**이다 — 평지와 판 양쪽에 접하며 **기울기가 0에서 판 기울기까지 단조 증가**한다(D-040 + D-041). 단조 조건 때문에 전이는 판 연장선보다 낮게 끝나므로 판 전체를 그 차이(`lift`)만큼 내려놓아야 이음새가 붙는다 — C0·C1 연속과 단조 기울기를 동시에 만족하는 유일한 방법이다(D-041). 기울기 축은 구간 진입점과 진출점을 잇는 선이라 두 점 높이가 같고 폐곡선이 닫힌다. 뱅크 구간은 색과 무관하게 **위로 솟는다**(D-037이 투영 깊이 부호를 정정한 뒤의 규약 — **양수 z가 화면 위**이며 그 이전 엔트리의 오르내림 표현은 반대 규약이다. D-031이 두 방향을 렌더해 비교한 뒤 확정한 그림이며, 본문의 "아래로 파임"은 투영 부호가 틀린 상태의 규약으로 쓴 표현이었다 — D-045가 정정). 웨이브(Chi*)는 고도 변화가 없고 **편집기 2D 좌표에서 `n = [−tan.y, tan.x]`의 양의 방향으로 5cm 곡면 돌출**하며 양 끝이 직선이다(D-032, 모양 `sin²(πt)`). **방향은 낱말이 아니라 이 벡터로 정한다** — "왼쪽/오른쪽"은 편집기 좌표에서 재느냐 투영된 화면에서 보느냐에 따라 갈려 실제로 프리뷰와 구현이 반대로 그려지는 사고를 냈다(2026-08-31). 레인체인지(Lan*)는 고도 변화가 없고 세 레인이 **한 칸씩 순환**(0→1, 1→2, 2→0)하며 교차한다 — 바깥 레인이 나머지 둘을 가로지른다(D-033, 공식 API의 order:[1,2,3] 근거). 레인은 선이 아니라 **면**으로 그린다 — 레인마다 12cm 폭을 가지며 전체 트랙 폭은 36cm다(D-034). 레인체인지에서 두 칸 건너뛰는 레인(3→1)은 나머지 둘 위로 넘어가는 **육교**이며 높이 8cm의 sin² 곡선으로 올라갔다 내려온다(D-035). 뱅크가 연속이면 기울기가 누적되나(+20°/−20°) **다단 기하는 미구현**이다(D-030). **뱅크 피스에 롤 값을 따로 얹지 않는다**(PC-011이 롤 모델을 세 번 고쳐 쓴 끝에 폐기했다). 다만 **판 구간의 노면은 좌우로 평평하지 않다** — 트랙이 20° 기운 판 **위를 돌면서** 지나가므로(D-029) 노면의 좌우 기울기는 판의 기하학적 귀결로 저절로 생긴다. 폐기된 것은 **따로 얹던 롤 값**이지 이것이 아니다. 종전 본문이 "노면은 좌우로 평평하다"고 적어 그 구분을 지웠고, 구현이 그 문장을 따라 판의 횡경사를 버렸다(2026-08-30 정정, FEAT-017). 판 위 임의 점의 높이는 그 점의 좌표로 정한다 — 중심선 높이를 좌우로 복사하는 것이 아니다. 고도는 **통과 방향**을 따라 깔린다 — 참조 트랙은 132피스 중 99개가 역방향 통과이며(PC-009), `flipped`를 무시하고 `vertex1→vertex2`로만 깔면 역방향 피스의 높은 쪽 끝이 반대편에 붙는다. `flipped`일 때 끝점을 맞바꾸면 코너 원호가 거울상이 되므로 **표본 배열만 뒤집는다**(`piece-geometry.md` §통과 방향). 슬로프의 **모양**은 양 끝 기울기 0의 S곡선 `h(x)=H/2·(1−cos(πx/L))`이다. 판은 평면이라 프로파일 함수를 쓰지 않고, 뱅크 피스만 전이 곡선을 쓴다(D-029가 로그 곡선 모델을, D-041이 3차 곡선 구현을 대체했다). 모양은 총 상승량을 바꾸지 않는다. S곡선이 양 끝 0°로 같은 높이를 내려면 중앙이 **약 52.8°**가 되는데 이는 모양의 수학적 귀결이지 지정값이 아니다. 이음매 접선 연속을 보장한다. 색상 규칙은 팔레트 인덱스 기준으로 좁혀 적용한다 — 고도 변화 피스(Bri*, Ban*)에 한해 c=3(빨강 계열)=상승, c=2(파랑 계열)=하강이다(measured, D-014 — decision-log N-001 해소; `c`는 방향 플래그가 아니라 팔레트 인덱스이므로 `Bri1.c0`=청록, `Str1.c6`=주황처럼 다른 의미로도 쓰인다). Str1의 c=5(마커 직선, REQ-F-020)는 고도 변화가 없는 출발선 표식이며 상승/하강 판정에서 제외하고 평평한 flat 프로파일로 렌더한다.
...
- TC-016-1: Given `Chi1` 피스, When 경로를 생성하면, Then 중앙에서 `n = [−tan.y, tan.x]` 방향으로 5cm 벗어나고 양 끝의 횡변위가 0이다(`sin²(πt)` 모양). **부호를 함께 단언한다** — 크기만 재면 반대로 그려도 통과한다.
- TC-016-2: Given `Chi*` 피스, When 고도 프로파일을 생성하면, Then 시작·끝·중간 모든 표본의 고도 변화가 0이다 — 웨이브는 평지다.
- TC-016-3: Given 역방향 통과(`flipped`) `Chi*` 피스, When 경로를 생성하면, Then 돌출 방향이 여전히 `n = [−tan.y, tan.x]`의 양의 방향이다(끝점 교환이 아니라 표본 뒤집기라 부호가 보존된다, PC-009).
