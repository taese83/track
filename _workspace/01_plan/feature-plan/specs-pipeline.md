...
### FEAT-005 — 고도 프로파일 생성 (슬로프 S곡선·뱅크 로그곡선)

**동작 명세**: 복원된 경로의 각 피스에 타입별 고도 프로파일 함수를 적용해 피스 내부를 곡면 세그먼트로 변환한다. 고도는 **현(chord) 규칙**으로 정한다(D-022, 사용자 지정 렌더 규칙 — 실물 실측 아님): 피스의 시작점과 끝점을 이은 직선이 슬로프는 **25°**(D-025), 뱅크는 **20°**를 이루도록 `H = 진행축 px × sin(각도)`로 총 상승량을 계산한다(D-023 — 진행축 px는 차가 달린 거리, 즉 빗변이다). 뱅크는 슬로프와 달리 곡선이 아니다. 상승 뱅크와 하강 뱅크 사이 구간 전체(뱅크 포함)가 **하나의 20° 기운 평면** 위에 놓이며, 트랙이 그 평면 위를 돌면서 올라갔다 내려온다(D-029). 기울기 축은 구간 진입점과 진출점을 잇는 선이라 두 점 높이가 같고 폐곡선이 닫힌다. 뱅크 구간은 색과 무관하게 **아래로 파인다**(D-031 — 두 방향을 렌더해 비교한 뒤 확정). 웨이브(Chi*)는 고도 변화가 없고 진행 방향 기준 **왼쪽으로 5cm 곡면 돌출**하며 양 끝이 직선이다(D-032). 레인체인지(Lan*)는 고도 변화가 없고 세 레인이 **한 칸씩 순환**(0→1, 1→2, 2→0)하며 교차한다 — 바깥 레인이 나머지 둘을 가로지른다(D-033, 공식 API의 order:[1,2,3] 근거). 레인은 선이 아니라 **면**으로 그린다 — 레인마다 12cm 폭을 가지며 전체 트랙 폭은 36cm다(D-034). 레인체인지에서 두 칸 건너뛰는 레인(3→1)은 나머지 둘 위로 넘어가는 **육교**이며 높이 8cm의 sin² 곡선으로 올라갔다 내려온다(D-035). 뱅크가 연속이면 기울기가 누적되나(+20°/−20°) **다단 기하는 미구현**이다(D-030). 롤(좌우 눕힘)은 쓰지 않는다 — 노면은 좌우로 평평하다. 슬로프의 **모양**은 양 끝 기울기 0의 S곡선 `h(x)=H/2·(1−cos(πx/L))`이다. 뱅크는 곡선이 아니라 평면이므로 프로파일 함수를 쓰지 않는다(D-029가 로그 곡선 모델을 대체했다). 모양은 총 상승량을 바꾸지 않는다. S곡선이 양 끝 0°로 같은 높이를 내려면 중앙이 **약 52.8°**가 되는데 이는 모양의 수학적 귀결이지 지정값이 아니다. 이음매 접선 연속을 보장한다. 색상 규칙은 팔레트 인덱스 기준으로 좁혀 적용한다 — 고도 변화 피스(Bri*, Ban*)에 한해 c=3(빨강 계열)=상승, c=2(파랑 계열)=하강이다(measured, decision-log N-001 해소; `c`는 방향 플래그가 아니라 팔레트 인덱스이므로 `Bri1.c0`=청록, `Str1.c6`=주황처럼 다른 의미로도 쓰인다). Str1의 c=5(마커 직선, REQ-F-020)는 고도 변화가 없는 출발선 표식이며 상승/하강 판정에서 제외하고 평평한 flat 프로파일로 렌더한다.

- TC-005-1: Given Bri1(slope) 피스와 앞뒤 평지 피스, When 고도 프로파일을 생성하면, Then 총 상승량이 진행축 px × sin25°와 일치하고 h(x)=H/2·(1−cos(πx/L)) 곡면이 생성되며 양 끝 접선 기울기가 0으로 앞뒤 평지와 매끄럽게 이어진다(이음매 접선각 오차 ±1° 이내, REQ-F-002).
- TC-005-6: Given Ban1(bank) 피스, When 고도 프로파일을 생성하면, Then 뱅크 구간의 모든 점이 하나의 평면 위에 있고(적합 잔차 ≈0) 그 평면의 기울기가 20°이며 구간 진입·진출 높이가 같다.
- TC-005-2: Given 상승 뱅크와 하강 뱅크로 둘러싸인 구간, When 고도를 생성하면, Then 그 구간의 모든 점이 하나의 평면 위에 놓이고(적합 잔차 ≤0.01cm) 평면 기울기가 20°이며 구간 진입·진출 높이가 같다(REQ-F-002).
