...
**트랙 벽의 렌더 단위**(PC-017, TC-020-6): 벽은 레인 면 396개에 **528개**를 더한다(참조 트랙
기준, 경계선과 같은 수). 정점 수는 대략 2배가 되지만 **draw call은 늘지 않는다** — 벽도
`buildTrackGeometries`로 색 단위 합침을 거치므로 색 종류(≤8)만큼이다. 이 항목이 예산에 미치는
영향은 정점·픽셀 비용이지 draw call이 아니다. **회귀 판정**: 벽 도입 전후를 같은 트랙에서
대조해 초기 렌더 3초·`orbitFps` 30 하한 유지를 증거로 남긴다.
