...
export function buildLanes(segments, opts = {}) {
  ...
  // 돌출 방향. **프리뷰가 그려 내는 이 방향이 확정값이다**(사용자 확인 2026-08-31).
  //
  // "왼쪽/오른쪽"이라는 낱말은 프레임에 따라 갈린다 — 편집기 2D 좌표에서 재는 것과
  // 투영(makeProjector)을 거친 화면에서 보는 것이 다르다. D-032 개정이 "반대로"라고 적었을
  // 때의 기준은 **화면에서 본 모습**이었고, 그것이 이 상수가 만들어 내는 그림이다.
  // 낱말로 다투지 않도록 **편집기 좌표의 법선으로 못박는다**:
  //
  //     tan = unit(p_next − p_prev),  n = [−tan.y, tan.x],  bow = sin²(πt) × 5cm × (+1)
  //
  // 즉 돌출은 `n` **양의 방향**이다. 구현(FEAT-016)은 이 벡터를 그대로 쓴다.
