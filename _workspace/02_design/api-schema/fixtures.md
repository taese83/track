...
**미녹화 코드는 "존재하지 않는 코드"가 아니다** (2026-08-30 추가). 아래 예약 코드에 없고
녹화 파일도 없는 코드는 `FIXTURE_NOT_RECORDED`(501)로 갈린다 — fixture 모드는 편집기에
묻지 않으므로 존재 여부를 알 수 없기 때문이다(§6). "존재하지 않는 코드"를 뜻하는 것은
예약 코드 `ZZZZZZ` 하나뿐이다. 종전 구현은 이 둘을 접어 실재하는 코드를 404로 냈다.
