...
| TrackCanvas(3D 씬) | widgets/track-canvas | bg-canvas(clearColor), rise/fall(표면 원본색), fail-segment, warning(미지원 플레이스홀더), primary(하이라이트 면 오버레이) + 3D 전용 파생 highlight-edge-light/dark(이중 테두리, FEAT-019·PC-016) |
