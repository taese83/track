...
- **트랙 벽**(FEAT-020, PC-017): 각 레인 경계에 노면에서 5cm 높이의 벽을 세운다. 줄 수·방향·색·미지원 처리의 정본은 `piece-geometry.md` §레인 경계에는 5cm 벽이 선다이며 여기에 옮겨 적지 않는다. 렌더 규칙만 적는다 — 벽은 레인 면과 같은 `meshStandardMaterial`·양면이고(두께가 없어 안쪽에서 보면 사라진다) **색 단위로 합쳐** draw call을 색 종류만큼으로 유지한다(`buildTrackGeometries` 재사용). 하이라이트(FEAT-019) 테두리는 `depthTest=false`라 벽 뒤에서도 보이므로 벽이 하이라이트를 가리지 않는다(TC-020-7).
