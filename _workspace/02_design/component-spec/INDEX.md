...
| 절 | 파일 | 담당 범위 | 주 소비자 |
|---|---|---|---|
| shared 레이어 + 공유 커서 계약 | `shared.md` | AlertSlot·StatusBanner/TopBar, **`useTrackCursor()` 소유권·통지·순환방지 계약(문서 핵심)** | developer, design-preview-builder |
...
- 절대 단위(m·cm) 표기 금지, 상승/하강 3중 인코딩(색+형태+텍스트) — 두 계약 모두 모든 절에서 반복 확인한다. ~~근거 등급 배지 4단계 상시 렌더~~는 PC-018로 제거됐다(design-system 결정 승계).
