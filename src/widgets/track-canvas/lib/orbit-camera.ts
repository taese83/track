 ...
 * 카메라가 튀므로 양 끝에 최소 여유만 둔다.
 *
 * **수평(π/2)에서 멈추지 않는다**(TC-006-6, PC-010 ②). 종전 주석은 "트랙은 바닥 위의
 * 물체이므로 밑에서 올려다보는 시점은 정보가 없다"고 적고 상한을 π/2로 잠갔는데, 그 전제가
 * 틀렸다 — 이 트랙은 육교·뱅크·입체교차로 **자기 위를 지나가는** 구조라 아래에서 봐야
 * 보이는 형상이 있다. 사용자가 프리뷰에서 "x y z 축 360도로"라고 확정하고 잠금을 걷어냈는데,
 * 그 조정이 명세에 도달하지 않아 구현이 잠금을 되돌린 채 머지됐다(2026-08-30 정정).
 */
const POLAR_EPSILON = 0.02
const MIN_POLAR_RAD = POLAR_EPSILON
const MAX_POLAR_RAD = Math.PI - POLAR_EPSILON
